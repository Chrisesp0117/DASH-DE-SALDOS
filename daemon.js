...
/**
 * Alert Daemon
 * Mantém o bot Telegram e scheduler rodando constantemente
 */
...
const { getMetaData } = require('./src/services/meta');
const { buildRow } = require('./src/core/calculator');
const { generateBlocosPorGestor } = require('./src/core/visualBlocks');
const { initTelegramBot } = require('./src/services/telegram');
const { scheduleAlerts } = require('./src/core/scheduler');
const { runChecks } = require('./src/validateKeys');
...
async function updateDatabase() {
  ...
  for (const row of clientes) {
    ...
    const plataforma = (row[1] || '').trim().toUpperCase();
    const id = String(row[2] || '').trim();
    const gestor = (row[3] || '').trim();
    const supervisor = (row[4] || '').trim();
    ...
    if (plataforma === 'GOOGLE') {
      if (!customerIdNormalized) {
        processStatus = 'Erro';
        obs = 'Customer ID ausente ou inválido';
      } else {
...
async function start() {
  try {
    ...
    console.log('\n🤖 Iniciando bot Telegram e scheduler...');
    const sheets = await getSheets();
    const bot = initTelegramBot(sheets, process.env.SPREADSHEET_ID);
    if (bot) {
      scheduleAlerts(sheets, process.env.SPREADSHEET_ID);
      console.log('✅ Bot Telegram aguardando comandos');
      console.log('📱 Comandos: /help, /exam, /atualizar');
      console.log('⏰ Relatórios automáticos: 8h e 17h (para usuários registrados)');
    }
